...
export type ContentCreatorDashboardTotals = {
...
export type ContentCreatorEnrollmentDayRow = {
  date: string;
  total: number;
};

export type ContentCreatorRecentCourseRow = {
...
export type GetContentCreatorDashboardOverviewInput = {
  enrollmentDays: number;
  recentCourseLimit: number;
  userId: string;
};

function emptyEnrollmentDays(enrollmentDays: number): ContentCreatorEnrollmentDayRow[] {
  const now = new Date();
  ...
  const enrollmentsByDay: ContentCreatorEnrollmentDayRow[] = [];
...
export async function getContentCreatorDashboardOverview(
  database: Database,
  input: GetContentCreatorDashboardOverviewInput,
) {
  const { enrollmentDays, recentCourseLimit, userId } = input;

  const instructedRows = await database
    .select({ courseId: courseInstructor.courseId })
    .from(courseInstructor)
    .where(eq(courseInstructor.userId, userId));

  const courseIds = [...new Set(instructedRows.map((row) => row.courseId))];
  if (courseIds.length === 0) {
    return {
      enrollmentsByDay: emptyEnrollmentDays(enrollmentDays),
      recentCourses: [] as ContentCreatorRecentCourseRow[],
      totals: {
        assignedCourses: 0,
        draftCourses: 0,
        publishedCourses: 0,
        totalEnrollments: 0,
      } satisfies ContentCreatorDashboardTotals,
    };
  }
