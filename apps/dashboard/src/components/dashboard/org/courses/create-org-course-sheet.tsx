...
import {
  ...
  createOrgCourseSchema,
  DEFAULT_CREATE_ORG_COURSE_VALUES,
  type CreateOrgCourseFormInput,
} from "./org-courses-schema";

const CREATE_ORG_COURSE_THUMBNAIL_INPUT_ID = "create-org-course-sheet-thumbnail-input";
...
type CreateOrgCourseSheetProps = {
  trigger?: ReactElement;
};

export function CreateOrgCourseSheet({ trigger }: CreateOrgCourseSheetProps) {
  const [open, setOpen] = useState(false);
  const trpc = useTRPC();
  const trpcClient = useTRPCClient();
  const queryClient = useQueryClient();
  const navigate = useNavigate();
  const listKey = trpc.orgCourse.list.queryKey();
  ...
  const form = useForm<CreateOrgCourseFormInput>({
    resolver: zodResolver(createOrgCourseSchema),
    defaultValues: DEFAULT_CREATE_ORG_COURSE_VALUES,
  });
  ...
  const onSubmit = async (data: CreateOrgCourseFormInput) => {
    try {
      const { courseId } = await trpcClient.orgCourse.create.mutate({
        title: data.title,
        slug: data.slug,
      ...
      if (coverFile) {
        try {
          ...
          await Promise.all([
            ...
            trpcClient.orgCourse.update.mutate({
              courseId,
      ...
      toastManager.add({
        title: "Course created",
        description: `${data.title} was added to this organization's catalog.`,
        type: "success",
      });

      setOpen(false);
      form.reset(DEFAULT_CREATE_ORG_COURSE_VALUES);
      setSlugTouched(false);
      setUploadProgresses({});
      setUploaderResetKey((k) => k + 1);
      await queryClient.invalidateQueries({ queryKey: listKey });
      await navigate({ to: "/dashboard/org/courses/$courseId", params: { courseId } });
    } catch (error) {
      const message =
      ...
      <SheetPopup className="max-w-2xl" side="right" variant="inset">
        <SheetHeader>
          <div className="space-y-1 pe-8">
            <SheetTitle>Create organization course</SheetTitle>
            <SheetDescription>
              Create a course for this organization. Add instructors and lessons from the course
              detail page.
            </SheetDescription>
          </div>
        </SheetHeader>
        ...
        <SheetPanel>
          <Form {...form}>
            <form
              ...
              id="create-org-course-sheet-form"
                ...
                render={({ field, fieldState }) => (
                  <FormItem>
                    <Field>
                      <FieldLabel
                        className="text-xs"
                        htmlFor={CREATE_ORG_COURSE_THUMBNAIL_INPUT_ID}
                      >
                        Thumbnail
                      </FieldLabel>
                      <FormControl>
                        <FileUploader
                          ...
                          inputId={CREATE_ORG_COURSE_THUMBNAIL_INPUT_ID}
                    ...
                    <Field className="w-full" invalid={!!fieldState.error}>
                      ...
                      {categoriesQuery.isLoading ? (
                        ...
                        <p className="text-sm text-muted-foreground">
                          No categories are available yet. You can still create the course without
                          categories.
                        </p>
                      ) : (
                        <FormControl>
        ...
        <SheetFooter variant="bare">
          ...
          <Button
            form="create-org-course-sheet-form"
