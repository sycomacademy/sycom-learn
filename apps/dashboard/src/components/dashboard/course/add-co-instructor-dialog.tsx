...
export function AddCoInstructorDialog({
  courseId,
  disabled,
}: {
  courseId: string;
  ...
  const candidateQuery = useQuery({
    ...trpc.course.listAvailableCoInstructors.queryOptions({
      courseId,
      limit: 20,
      offset: 0,
  ...
  const handleAdd = async (userId: string) => {
    try {
      await trpcClient.course.addCoInstructor.mutate({ courseId, userId });
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: trpc.course.listCoInstructors.queryKey({ courseId }),
        }),
        queryClient.invalidateQueries({
          queryKey: trpc.course.listAvailableCoInstructors.queryKey(),
        }),
      ]);
    ...
    <Dialog onOpenChange={handleOpenChange} open={open}>
      ...
      <DialogPopup className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose a platform admin or content creator. Courses can have up to 3 co-instructors.
          </DialogDescription>
        </DialogHeader>
