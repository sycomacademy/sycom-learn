...
export const Route = createFileRoute("/dashboard/org/library/")({
...
function OrgLibraryPage() {
  const trpc = useTRPC();
  const { data } = useSuspenseQuery(trpc.student.getLibrary.queryOptions({}));

  return (
    <FadeIn className="flex flex-col gap-6 px-6 py-6" motionKey="org-library">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold tracking-tight">My library</h1>
        <p className="text-sm text-muted-foreground">
          Org courses you&apos;re enrolled in, with progress and cohort context.
        </p>
      </div>
    ...
    return (
      <Card className="shadow-xs">
        <CardHeader>
          ...
          <CardDescription>
            Every org course you’re enrolled in, ordered by your latest activity.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <OverviewListEmpty
            description="When you’re enrolled in an org course, it will show up here."
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-wrap items-end justify-between gap-2 px-4 pt-12 pb-3">
        <div className="flex min-w-0 flex-col gap-1">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {course.cohortNames.length > 0 ? (
              <Badge size="sm" variant="secondary">
                {course.cohortNames[0]}
              </Badge>
            ) : null}
            {course.cohortNames.length > 1 ? (
              <Badge size="sm" variant="outline">
                +{course.cohortNames.length - 1} more
              </Badge>
            ) : null}
          </div>
          {course.startedAt ? (
