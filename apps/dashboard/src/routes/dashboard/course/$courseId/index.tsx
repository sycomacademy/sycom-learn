...
function CourseDetailsPage() {
  ...
  const onSubmit = async (data: EditCourseFormInput) => {
    ...
    try {
      if (coverImage) {
        ...
        await Promise.all([
          ...
          ...(Object.keys(patch).length > 0
            ? [trpcClient.course.update.mutate({ courseId, patch })]
            : []),
          ...(categoriesChanged
            ? [trpcClient.course.setCategories.mutate({ courseId, categoryIds: data.categoryIds })]
            : []),
        ]);
      } else {
        const operations = [
      ...
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: trpc.course.get.queryKey({ courseId }) }),
